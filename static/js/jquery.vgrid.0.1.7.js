...
(function($)
{
	function makePos(self)
	{
      ...
      var imageWidth = parseInt(bodyWidth / numberChilds);

    
		_childs.each(function(i)
		{
			_c = $(this);
			_c.css({'position':'absolute','left':_c.data("_vgleft"),'top':_c.data("_vgtop")});
      ...
      var _size = new Array();
	  // handleimagesize  处理图片尺寸
	  if(_c.hasClass('big')) {   //a带big的css
		_size[0] = imageWidth * 2+1;
	    _size[1] = imageWidth * 2 / tmpSize[0] * tmpSize[1]+1;
	  } else {
		_size[0] = imageWidth+1;
	    _size[1] = imageWidth+1;		
	  }
  _point = getAttachPoint(_matrix, _size[0]);   //_point 包含 top 和 left 偏移量
      ...
      _c.data("_vgwidth", _size[0]);
      _c.data("_vgheight", _size[1]);
      
      _c.find('img').attr('width', _size[0]);
      _c.find('img').attr('height', _size[1]);
		});
